...
## New Native Agent Architecture (2026-07)

OneAgent now ships with a unified Python core that borrows the best patterns from 14 major open-source AI agent frameworks:

| Module | Inspiration | Purpose |
|---|---|---|
| `core/llm/` | GPT Researcher, Goose, Cline | Provider registry + `provider:model` resolution |
| `core/tools/` | OpenManus, smolagents, Cline | Declarative `BaseTool`, `ToolCollection`, `tool()` factory |
| `core/agent/` | OpenManus | Harness ladder: BaseAgent → ReActAgent → ToolCallAgent |
| `core/scraper/` | Firecrawl | Pluggable scrape engines (fetch, playwright) with fallback |
| `core/coding/` | Aider, Anthropic | `read_file`, `shell`, `str_replace_editor` tools |
| `core/skills/` | GPT Researcher | Composable skills: research, coding, browser |
| `core/context/` | Gemini CLI, Cline | Token budget tracking + context compression |
| `core/policy/` | Gemini CLI, Goose | Per-tool ALLOW / DENY / ASK_USER policy engine |
| `core/scheduler/` | Gemini CLI, Super-App | Async scheduled task runner |
| `core/workspace/` | OpenClaw | `SOUL.md`, `AGENTS.md`, memory files |
| `core/session/` | OpenClaw | JSONL transcripts + compaction |
| `core/security/` | Super-App, Goose | Command / URL / file-path validation |
| `core/recipe/` | Super-App | Multi-step skill chains with `continue_on_error` |
| `core/harness/` | OpenClaw | Pluggable model harness registry |
| `core/subagent/` | OpenManus, Goose | Spawn background sub-agents |
| `core/hooks/` | OpenClaw, Gemini CLI | Lifecycle hook system |
| `core/diagnostics/` | OpenClaw | Flags, timeline, liveness classification |
| `core/capabilities/` | OpenClaw | Capability registry |
| `core/queue/` | OpenClaw | Steering + followup queues |

### New UI views

- **Skill Runner** — run native research / coding / browser skills.
- **Web Scraper** — Firecrawl-style scrape with JavaScript/screenshot toggles.
- **Coding Agent** — Aider-style agent harness with read/edit/shell tools.
- **Agent Architecture** — overview of harnesses, capabilities, hooks, sessions, recipes, security, diagnostics.

### Backend API additions

- `GET /api/providers` — list LLM providers.
- `POST /api/skills/:name/run` — run a native skill.
- `POST /api/coding/run` — run coding specialist agent.
- `POST /api/scrape` — scrape a URL.
- `POST /api/research/run` — run research skill.
- `GET /api/tools` — list built-in tools.
- `POST /api/policy/check` — check a tool against policy.
